...
class DataLoader(object):
  ...
  def train_get_batch_from_indices(self, indices):
    ...
    pad_data,pad_labels,pad_str_labels,new_seq_len,pad_stroke_nums = self.train_pad_batch(x_batch, self.max_seq_length,seq_len)
    #seq_len = np.array(seq_len, dtype=int)
    return x_batch, pad_data,pad_labels,pad_str_labels, new_seq_len,pad_stroke_nums
  ...
  def test_get_batch_from_indices(self, indices):
    ...
    pad_data,pad_labels,pad_str_labels,pad_stroke_nums = self.test_pad_batch(x_batch, self.max_seq_length)
    seq_len = np.array(seq_len, dtype=int)
    return x_batch, pad_data,pad_labels,pad_str_labels, seq_len,pad_stroke_nums

  def random_batch(self):
    """Return a randomised portion of the training data."""
    idx = np.random.permutation(range(0, len(self.strokes)))[0:self.batch_size*5]
    # idx = np.arange(self.batch_size*5)
  ...
  def train_pad_batch(self, batch, max_len,seq_len):
    ...
    for i in range(len(batch)):
      ...
      l = len(batch[i])
      assert l <= max_len
      #current_stroke_num=batch[i][0,3]
      #new_stroke_num = 0
      ...
      for line_indx in range(l):
        ...
        #if batch[i][line_indx,3]==current_stroke_num:
        #  stroke_nums[line_indx] = new_stroke_num
        #else:
        #  stroke_nums[line_indx]
      ...
      if ii<self.batch_size:
        ...
        return result, labels, gap_seg_labels, new_seq_len,pad_stroke_nums
